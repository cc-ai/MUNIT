...
# Loop and run the sampler and the net until it accumulates inception_images
# activations. Return the pool
def accumulate_inception_activations(trainer,fid_loader, net):
    pool = []
    for it, (images_a_fid, images_b_fid) in enumerate(fid_loader):
        images_a_fid,images_b_fid=images_a_fid.cuda(),images_b_fid.cuda()
        with torch.no_grad():
            batch_fake = trainer.sample_fid(images_a_fid,images_b_fid)
            pool_val= net(batch_fake)
            pool += [pool_val]
...
def prepare_inception_metrics(inception_moment,parallel=False):
    # Load metrics
    data_mu = np.load(inception_moment)['mu']
    data_sigma = np.load(inception_moment)['sigma']
    # Load network
    net = load_inception_net(parallel)
    def get_inception_metrics(trainer, fid_loader,prints=True, use_torch=True):
        if prints:
            print('Gathering activations...')
        pool = accumulate_inception_activations(trainer,fid_loader, net)
        if prints:
