...
parser.add_argument(
    "--config",
    type=str,
    default="configs/edges2handbags_folder",
    ...
    for i, (images, names) in enumerate(zip(data_loader, image_names)):
        ...
        for j in range(opts.num_style):
            ...
            if opts.compute_IS or opts.compute_CIS:
                pred = (
                    F.softmax(inception(inception_up(outputs)), dim=1)
                    .cpu()
                    .data.numpy()
                )  # get the predicted class distribution
            if opts.compute_IS:
        ...
        if not opts.output_only:
            ...
            vutils.save_image(
                images.data,
                os.path.join(opts.output_folder, "input{:03d}.jpg".format(i)),
                padding=0,
                normalize=True,
            )
    if opts.compute_IS:
